...
gene_list_hindbrain_liv_df <- system.file("extdata", "gene_list_hindbrain_liv_df.RData", package = "profileplyr") 

gene_list_dataframe <- readRDS(gene_list_hindbrain_liv_df) 
...
```{r, fig1, fig.height=7, fig.width=5, results='hide', fig.keep="last", warning=FALSE}
...
# Export/Conversion of profileplyr object for visualization
...
2) [A list of matrices that can be used in the EnrichedHeatmap() function within R](#convertToEnrichedHeatmapMat)
3) [A heatmap containing all range grouping and annotation information (using a modified implementation of the EnrichedHeatmap constructor)](#generateEnrichedHeatmap)
...
## Summarizing signal for ggplot or heatmap visualization
...
### Matrix output with the summarize() function
...
### Long output for ggplot with the summarize() function
...
### profileplyr object output with the summarize() function
...
```{r, fig11, fig.height=8, fig.width=5}
...
```{r, echo=F}
unlink("great_groupByGene_HB_Liv.gz")
```
...
<img src="great_groupByGene_HB_Liv.jpg" width="300">
...
```{r, fig14, fig.height=8, out.width='100%', results='hide', dpi=35}
# NOTE: could also use BamBigwig_to_chipProfile() and as_profileplyr()
# within R to start from bigwig/BAM and BED file. 
