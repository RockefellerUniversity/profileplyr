...
export_deepToolsMat(proplyrObject, con = "./groupByGene_HB_Liv")
```

The output from the previous code chunk can then be used to produce the following figure using the deepTools 'plotHeatmap' function.
...
Since the profileplyr object is used as both an input and output for most functions within the package, using the pipe (%>%) from the magrittr package can make the code much simpler. This is shown in the following two code chunks. 
